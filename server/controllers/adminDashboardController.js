...
const getAllByRole = (role) => async (req, res) => {
  try {
    const filter =
      role === "candidate"
        ? { role: { $in: ["candidate", "user"] } }
        : { role };
    const users = await Login.find(filter).select("userName email role status");
    res
      .status(200)
      .json({ error: false, message: "Operation successful", data: users });
  } catch (error) {
...
const createAccountOfRole = (role) => async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      return res
        .status(409)
        .json({ error: true, message: "Email already registered", data: null });
    }
...
const updateAccountOfRole = (role) => async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      return res
        .status(400)
        .json({ error: true, message: "Invalid userId format", data: null });
    }
    const account = await Login.findById(userId);
    if (!account)
      return res
        .status(404)
        .json({ error: true, message: "Account not found", data: null });

    const isCandidate =
      role === "candidate" && ["candidate", "user"].includes(account.role);
    if (!isCandidate && account.role !== role) {
      return res
        .status(403)
        .json({ error: true, message: `User is not a ${role}`, data: null });
    }
    ...
    const updated = await Login.findByIdAndUpdate(userId, updates, {
      new: true,
      runValidators: true,
...
const deleteAccountOfRole = (role) => async (req, res) => {
  try {
    ...
    if (!userId)
      return res
        .status(400)
        .json({ error: true, message: "userId is required", data: null });
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      return res
        .status(400)
        .json({ error: true, message: "Invalid userId format", data: null });
...
// ─── Candidate CRUD ───────────────────────────────────────────────────────────
exports.getCandidatesData = getAllByRole("candidate");
...
// ─── Recruiter CRUD ───────────────────────────────────────────────────────────
exports.getRecruitersData = getAllByRole("recruiter");
